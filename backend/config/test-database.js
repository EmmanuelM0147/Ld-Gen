...
// Test Supabase PostgreSQL connection using test config
...
const testConnection = async () => {
  try {
    ...
    client.release();
    console.log('✅ Test database connected successfully:', result.rows[0].current_time);
    return true;
  } catch (error) {
...
const initializeAllTables = async () => {
  try {
    ...
    // Insert test data
    for (const lead of testConfig.testData.sampleLeads) {
      await pool.query(`
        INSERT INTO business_contacts (company_name, industry, city, status) 
        VALUES ($1, $2, $3, $4)
        ON CONFLICT DO NOTHING
      `, [lead.company_name, lead.industry, lead.city, lead.status]);
    }
    ...
    console.log('\n🧪 Test data inserted:', testConfig.testData.sampleLeads.length, 'sample leads');
